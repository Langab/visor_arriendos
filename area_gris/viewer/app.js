/* ===================================================================
   ÁREA GRIS — Visor de Arriendos · lógica de UI
   Vistas: Lista · Mapa · Métricas · Ofertas · Análisis temporal
   Estado persistido en localStorage: contactadas y favoritas.
   (claves con prefijo areagris_ para no chocar con el visor principal,
    que vive en el mismo dominio)
   =================================================================== */
(() => {
  "use strict";
  ...
  // ---------- estado persistido ----------
  const LS_CONTACT = "areagris_contactadas";
  const LS_FAV = "areagris_favoritos";
  ...
  // Semáforo Área Gris: calza = dentro de presupuesto Y sin extras
  // (sin amoblar, sin estacionamiento, sin bodega); parcial = solo presupuesto.
  function nivel(l) {
    if (l.dentro_presupuesto && l.sin_extras) return "good";
    if (l.dentro_presupuesto) return "mid";
    return "bad";
  }

  // ---------- estado de filtros ----------
  const PMAX = Math.max(META.presupuesto_max || 350000, 350000);
  const ARR_INF = 600000;   // sentinel "sin límite" del slider de arriendo
  const GC_INF = 200000;    // sentinel "sin límite" del slider de gastos comunes
  const F = {
    texto: "", precioMax: PMAX, arriendoMax: ARR_INF, gcMax: GC_INF,
    dormMode: "", metroMax: 2000, supMin: 0, antiguedad: "", moneda: "",
    comunas: new Set(), barrios: new Set(), fuentes: new Set(),
    soloPresupuesto: false, soloBarrio: false, mascotas: false,
    sinAmoblar: false, sinEstac: false, sinBodega: false,
    soloOfertas: false,
  ...
  function aplicar() {
    let out = LISTINGS.filter((l) => {
      ...
      if (F.dormMode === "solo1" && !(l.dormitorios == null || l.dormitorios <= 1)) return false;
      if (F.dormMode === "2mas" && (l.dormitorios || 0) < 2) return false;
      ...
      if (F.sinEstac && l.tiene_estacionamiento) return false;
      if (F.sinBodega && l.tiene_bodega) return false;
  ...
  function tarjeta(l) {
    ...
    // lo que Área Gris NO busca, marcado en ámbar; sin amoblar confirmado, en verde
    if (l.amoblado === true) specs.push(`<span class="spec tag-warn">amoblado</span>`);
    if (l.amoblado === false) specs.push(`<span class="spec tag-ok">sin amoblar</span>`);
    if (l.tiene_estacionamiento) specs.push(`<span class="spec tag-warn">estac.</span>`);
    if (l.tiene_bodega) specs.push(`<span class="spec tag-warn">bodega</span>`);
  ...
  function pintarMapa(items) {
    ...
    conGeo.forEach((l) => {
      ...
      m.bindPopup(`
        <div class="pop">${img}
          <div class="pop-b">
            <div class="pop-price">${l.moneda === "UF" && l.precio_uf != null ? "UF " + l.precio_uf : clp(l.precio_clp)} <small style="font-size:11px;color:#737379">/mes · total ${clp(l.total_estimado_clp)}</small></div>
  ...
  function pintarMetricas() {
    ...
    const limpios = L.filter((x) => x.sin_extras).length;
    const formato1 = L.filter((x) => x.formato_1p).length;
    const barrio = L.filter((x) => x.en_barrio_objetivo).length;
    const sinAmoblar = L.filter((x) => x.amoblado !== true).length;
    ...
    const funnel = `
      <div class="metric-card wide">
        <h3>¿Cuántas se ajustan a lo que busca Área Gris?</h3>
        <div class="funnel">
          <div class="fn"><b>${total}</b><span>avisos totales</span></div>
          <div class="fn"><b>${presu}</b><span>≤ $${(META.presupuesto_max||350000).toLocaleString("es-CL")} total</span></div>
          <div class="fn"><b>${limpios}</b><span>sin amoblar · sin estac. · sin bodega</span></div>
          <div class="fn"><b>${barrio}</b><span>en barrio objetivo</span></div>
          <div class="fn hl"><b>${match}</b><span>◼ match Área Gris</span></div>
        </div>
      </div>`;
    ...
    const porPrecio = [
      ["≤ 250k", L.filter((x) => x.total_estimado_clp <= 250000).length],
      ["250–300k", L.filter((x) => x.total_estimado_clp > 250000 && x.total_estimado_clp <= 300000).length],
      ["300–350k", L.filter((x) => x.total_estimado_clp > 300000 && x.total_estimado_clp <= 350000).length],
      ["> 350k", L.filter((x) => x.total_estimado_clp > 350000).length],
    ];
    ...
    $("#metrics").innerHTML = `
      <div class="metrics-grid">
        ${funnel}
        <div class="kpis">
          ${kpi(match, "◼ Match Área Gris", "hl")}
          ${kpi(META.nuevos_desde_anterior || 0, "✦ Nuevas desde la última act.")}
          ${kpi(sinAmoblar, "Sin amoblar (o sin dato)")}
          ${kpi(formato1, "Studio o 1 dormitorio")}
  ...
  function render() {
    ...
    count.innerHTML = `<b>${items.length}</b> propiedades · ${calzan} ◼ match Área Gris`;
  ...
  function pintarOfertas() {
    ...
    // ofertas dentro de lo que busca Área Gris (presupuesto)
  ...
  function pintarTemporal() {
    ...
    const serieHTML = serie.length ? `
      <div class="metric-card wide">
        ...
        ${miniSerie("Match Área Gris ◼", serie.map((s) => s.match_perfecto), fechasBar)}
        ${miniSerie("Con 1+ dormitorios", serie.map((s) => s.tres_dorms), fechasBar)}
  ...
  function montarFiltros() {
    ...
    $("#f-sinestac").onchange = (e) => { F.sinEstac = e.target.checked; render(); };
    $("#f-sinbodega").onchange = (e) => { F.sinBodega = e.target.checked; render(); };
    ...
    $("#reset").onclick = () => {
      Object.assign(F, {
        texto: "", dormMode: "", metroMax: 2000, supMin: 0, antiguedad: "", precioMax: PMAX,
      ...
      ["#f-sinamoblar", "#f-sinestac", "#f-sinbodega", "#f-presupuesto", "#f-barrio-obj",
  ...
  function init() {
    if (!LISTINGS.length) {
      $("#grid").innerHTML = `<div class="empty"><h3>No hay datos todavía</h3><p>Corre <code>python area_gris/run_all.py</code> para generar la base.</p></div>`;
      return;
    }
